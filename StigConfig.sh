...
if [[ "$checkcomp" -ge 0 ]]; then
 echo "System is not compliant as users do not require complex passwords!"
        ...
        if [[ $userinput2 == [yY] || $userinput2 == [yY][eE][sS] ]]; then
                ...
                finalcomp=($(grep -i "ocredit" /etc/security/pwquality.conf| cut -d " " -f 3))
                echo "The new complexity is" $finalcomp "and the system is now compliant!"
        else
...
if [[ "$checkapp" != "apparmor" ]]; then
	...
	if [[ $userinput5 == [yY] || $userinput5 == [yY][eE][sS] ]]; then
        ...
        if [[ "$checkarmor" != "enabled" ]]; then
                read -p "The apparmor service is installed but inactive. Would you like to activate >
                ...
                echo "System is compliant with Apparmor. Onto STIG Check 6!"
        fi
fi
...
if [[ "$checkpasswd" != "L" ]]; then
        ...
        read -p "Would you like to prevent users from directly logging into the root user y/n: " use>
        if [[ $userinput7 == [yY] || $userinput7 == [yY][eE][sS] ]]; then
                sudo passwd -l root
                echo "Users can no longer directly login to the root user! STIG Checklist is complet>
        else
                echo "STIG Checklist is complete. Have a good day!"
        fi
else
        echo "The system is compliant and nobody can log directly into the root user. Your system is>
        echo "Have a great day!"
fi
#reboots the system to automatically apply all the new changes on the system for all users. Thank yo>
sudo reboot
